            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Catatan</th>
                </tr>
              </thead>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <input
                        ...
                        value={row.notes}
                        disabled={isReadOnly}
                        onChange={(e) => scoring.updateScore(row.id, row.score, e.target.value)}
                        className="w-full text-xs rounded-xl border border-slate-200 px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 disabled:bg-slate-50 disabled:text-slate-400"
                        id={`wudu-note-${row.id}`}
                      />
                    </td>
                  </tr>
            ...
            {scoring.rows.map((row, idx) => (
              ...
              >
                ...
                <input
                  ...
                  disabled={isReadOnly}
                  onChange={(e) => scoring.updateScore(row.id, row.score, e.target.value)}
                  className="w-full text-xs rounded-xl border border-slate-200 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 disabled:bg-slate-100 disabled:text-slate-400"
                  id={`wudu-note-mobile-${row.id}`}
                />
              </div>
            ))}
